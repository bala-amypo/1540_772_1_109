...
@Service 
@Transactional 
public class RecommendationEngineServiceImpl implements 
...
cardRepo.findActiveCardsByUser(intent.getUserId()); 
        ...
        for (CreditCardRecord card : activeCards) { 
            List<RewardRule> rules = ruleRepo.findActiveRulesForCardCategory(card.getId(), 
intent.getCategory()); 
            if (rules != null) { 
        ...
        if (bestCard == null) { 
            throw new BadRequestException("No valid reward rule found for category: " + 
intent.getCategory()); 
        } 
...
"}"); 
    ...
    public RecommendationRecord getRecommendationById(Long id) { 
        return recRepo.findById(id) 
                .orElseThrow(() -> new ResourceNotFoundException("Recommendation not 
found")); 
    } 
